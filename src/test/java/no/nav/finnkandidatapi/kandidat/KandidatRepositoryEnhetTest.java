...
public class KandidatRepositoryEnhetTest {
    ...
    public void setUp() {
        ...
        when(jdbcTemplate.queryForObject(any(), any(), eq(kandidatMapper))).thenReturn(TestData.enKandidat());
    ...
    public void slettKandidat__skal_sette_veileder_som_brukertype() {
        repository.slettKandidat(TestData.enKandidat().getAktørId(), TestData.enVeileder(), TestData.now());
    ...
    public void slettKandidatSomMaskinbruker__skal_sette_maskin_som_brukertype() {
        repository.slettKandidatSomMaskinbruker(TestData.enKandidat().getAktørId(), TestData.now());
        verify(jdbcInsert, times(1)).executeAndReturnKey(requestCaptor.capture());
