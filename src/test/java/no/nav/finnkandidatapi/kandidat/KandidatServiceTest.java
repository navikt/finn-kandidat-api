...
import no.nav.finnkandidatapi.unleash.UnleashConfiguration;
...
public class KandidatServiceTest {
    ...
    @Before
    public void setUp() {
        when(featureToggleService.isEnabled(UnleashConfiguration.HENT_OPPFØLGINGSBRUKER_VED_OPPRETT_KANDIDAT)).thenReturn(true);
    ...
    public void opprettKandidat__skal_opprette_kandidat_med_riktige_felter() {
        ...
        Kandidat kandidatTilOpprettelse = Kandidat.opprettKandidat(
                ...
                TestData.etNavKontor()
        );

        when(dateProvider.now()).thenReturn(kandidat.getSistEndret());
        when(veilarbArenaClient.hentOppfølgingsbruker(kandidat.getFnr(), kandidat.getAktørId())).thenReturn(TestData.enOppfølgingsbruker());
    ...
    public void opprettKandidat__skal_publisere_KandidatOpprettet_event() {
        ...
        kandidatService.opprettKandidat(kandidatDto, TestData.enVeileder());
    ...
    public void endreKandidat__skal_endre_kandidatens_felter() {
        ...
        Veileder veileder = TestData.enVeileder();
        LocalDateTime datetime = TestData.now();
    ...
    public void endreKandidat__skal_returnere_empty_hvis_ingen_kandidat() {
        ...
        Optional<Kandidat> endretKandidat = kandidatService.endreKandidat(kandidatDto, TestData.enVeileder());
    ...
    public void endreKandidat__skal_returnere_empty_hvis_man_ikke_kunne_lagre_kandidat() {
        KandidatDto kandidatDto = TestData.enKandidatDto();
        when(repository.hentNyesteKandidat(kandidatDto.getAktørId())).thenReturn(Optional.empty());
        Optional<Kandidat> endretKandidat = kandidatService.endreKandidat(TestData.enKandidatDto(), TestData.enVeileder());
        assertThat(endretKandidat).isEmpty();
    }
    ...
    public void endreKandidat__skal_publisere_Kandidatoppdatering_event() {
        ...
        Kandidat endretKandidat = kandidatService.endreKandidat(TestData.enKandidatDto(), TestData.enVeileder()).get();
    ...
    public void kandidatEksisterer__skal_returnere_true_hvis_kandidat_eksisterer() {
        ...
        when(repository.hentNyesteKandidat(aktørId)).thenReturn(Optional.of(TestData.enKandidat()));
